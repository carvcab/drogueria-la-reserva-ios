...
struct Customer: Identifiable, Codable, Hashable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // creditLimit safety
        if let clVal = try? container.decode(Double.self, forKey: .creditLimit) {
            self.creditLimit = clVal
        } else if let clVal = try? container.decode(Int.self, forKey: .creditLimit) {
            self.creditLimit = Double(clVal)
        } else {
            self.creditLimit = 0.0
        }
        
        // balance safety
        if let bVal = try? container.decode(Double.self, forKey: .balance) {
            self.balance = bVal
        } else if let bVal = try? container.decode(Int.self, forKey: .balance) {
            self.balance = Double(bVal)
        } else {
            self.balance = 0.0
        }
    }
